...
\textbf{Part (c):} The linear combination of two points in $S$ is also in $S$, so the inverse sum is a convex set. \\
\textbf{Part (d):} The inverse sum of $S_1$ and $S_2$ is
\[
    S = \bigcup_{1 \geq \lambda \geq 0} \{\lambda (1, 1)^T \, \cap \, (1 - \lambda)((2, 2)^T + u) \bigm\vert \norm{u} \leq 1\}
\]
