...
%\def 
Let $f : \mathcal{U} \to \mathcal{U}$. $u^*$ is a fixed point of $f(\dot)$ if $f(u^*) = u^*$.

%\def 
Let $f : \mathcal{U} \to \mathcal{U}$ and let $\norm{\dot}$ be a norm defined on $\mathcal{U}$. We say $f(\dot)$ is a contraction mapping (or simply, 
...
the sequence converges, i.e., there exists a limit $u^* = \lim_{k \to \infty} u_k$. Since $f(\dot)$ is a contraction, 
...
\[
  V(s) = r(s,a) + \gamma \E_{s' \sim P(\dot | s,a)} [V^*(s')]
\]
...
\begin{align*}
  |(TV_1)(s) - (TV_2)(s)| &= |\max_{a \in \mathcal{A}}(r(s,a) + \gamma \E_{s' \sim P(\dot | s, a)}[V_1(s')]) - \max_{a \in \mathcal{A}}(r(s,a) + \gamma \E_{s' \sim P(\dot | s, a)}[V_2(s')])|
  &\leq \max_{a \in \mathcal{A}} |\gamma \E_{s' \sim P(\dot | s, a)}[V(s')]) - ||\gamma \E_{s' \sim P(\dot | s, a)}[V(s')])\gamma \E_{s' \sim P(\dot | s, a)}[V_2(s')])| 
  &= \\ 
  &\leq \max_{a \in \mathcal{A}} \gamma \E_{s' \sim P(\dot | s,a)} [|V_1(s') - V_2(s')|] \\
  &\leq \max_{a \in \mathcal{A}} \gamma \E_{s' \sim P(\dot| s,a)} [\norm{V_1 - V_2}_{\infty}] = \gamma \norm{V_1 - V_2}_{\infty}
\end{align*}
this implies 
\[
  \norm{TV_1 - TV_2}_{\infty} \leq \gamma ...
